...
class NNResult{
public:
	static void free_an_NNResult_vec(Vector<NNResult<T>*> NN_result_arr){
		NNResult<T>* first = (NNResult<T>*)NN_result_arr[0];
		first->free_stru();
	}
...
public:
	...
	void new_kNN(Matrix<T> &query_points_mat, int K, FLOAT_TYPE* dist_square_k_mins_global, int* idx_k_mins_global, int* remain_index);
	...
	Matrix<T>  &getOutliers(){ return this->all_exc_data; };
	Vector<int>&getOutliers_indexes(){ return this->all_exc_indexes; };
	...
	void print_kNN_reult(Matrix<T>& query_points_mat){
...
protected:
    //this should be implemented in subclass
	//brute force computing and update dist_k_mins_global and idx_k_mins_global from all_exc_data
    ...
    */
	...
	CYW_TIMER timer_do_KNN;
	CYW_TIMER timer_update_KNN_from_outliers;
	...
	//finding outliers from data by FastDPeak
	Vector<int> all_exc_indexes;
	Matrix<T> all_exc_data;

    //after building bkm tree, the data will be rearranged according to the order of leaf nodes
...
private:
	...
	void new_init_process_query_points(Matrix<T> &query_points_mat, int K, FLOAT_TYPE* dist_square_k_mins_global, int* idx_k_mins_global);
    /*------------------------------init query points and K---------------------------*/
    ...
    void sort_raw_data();
	//sort m_data without outliers after bkm tree is built by the order of leave nodes
	void sort_raw_data_without_outliers();
...
void Convex_Tree<T>::init_process_query_points(Matrix<T> &query_points_mat, int K){
	...
	this->init_kNN_result(query_points_mat);
    this->timer_init_process_query_points.stop_my_timer();
	timer_init_process_query_points.print("init_process_query_points time:");
}

template <typename T>
void Convex_Tree<T>::new_init_process_query_points(Matrix<T> &query_points_mat, int K, FLOAT_TYPE* dist_square_k_mins_global, int* idx_k_mins_global){
	this->timer_init_process_query_points.start_my_timer();
	this->K = K;
	//this is virtual procedure for subclass to override it.
	this->new_init_kNN_result(query_points_mat,dist_square_k_mins_global, idx_k_mins_global);
	this->timer_init_process_query_points.stop_my_timer();
	timer_init_process_query_points.print("\ninit_process_query_points time:");
}
...
void Convex_Tree<T>:: kNN(Matrix<T> &query_points_mat, int K){
    ...
    this->clearNN_info();
	printf("\nConvex_Tree::init_process_query_points ...");
    this->init_process_query_points(query_points_mat,K);
	printf("\nConvex_Tree::init_process_query_points completed...");
    //virtual procedure that need to be override in subclass
	printf("\ncall k_NN ...");
	this->timer_do_KNN.start_my_timer();
    this->do_kNN(query_points_mat);
	this->timer_do_KNN.stop_my_timer();
	this->timer_do_KNN.print("\ndo KNN  time is:");
	this->timer_whole_alg.stop_my_timer();
	printf("\nConvex_Tree::KNN over...");
	this->timer_whole_alg.print("\nwhole time is:");
}

//the entry for perform kNN
template <typename T>
void Convex_Tree<T>::new_kNN(Matrix<T> &query_points_mat, int K, FLOAT_TYPE* dist_square_k_mins_global, int* idx_k_mins_global,int* remain_index){
	int dim = query_points_mat.ncols();
	if (dim != this->m_data.ncols()){
		std::cout << "the dim of query points is " << dim << ", while the dim of data set is " << m_data.ncols();
		return;
	...
	this->clearNN_info();
	printf("\nConvex_Tree::init_process_query_points ...");
	this->new_init_process_query_points(query_points_mat, K, dist_square_k_mins_global, idx_k_mins_global);
	//this->init_kNN(dist_square_k_mins_global, idx_k_mins_global);
	printf("\nConvex_Tree::init_process_query_points completed...");
	//virtual procedure that need to be override in subclass
	printf("\ncall k_NN ...");
	this->timer_do_KNN.start_my_timer();
	this->new_do_kNN(query_points_mat,remain_index);
	this->timer_do_KNN.stop_my_timer();
	this->timer_do_KNN.print("\ndo KNN  time is:");
	this->timer_whole_alg.stop_my_timer();
	printf("\nConvex_Tree::KNN over...");
...
template<typename T>
Convex_Tree<T>::Convex_Tree(Matrix<T>& data, FLOAT_TYPE leaf_pts_percent,
                            int alg_type):m_data(data),leaf_pts_percent(leaf_pts_percent),ALG_TYPE(alg_type)
{
    ...
    //build tree
    build_tree(this->depth,alpha,0,-1,index_vec,cur_center);
	//Update leaf nodes by FastDPeak
	//Update_leaf_nodes();
    ...
    //rearrange the raw data, result in sorted_data
    sort_raw_data();
	//rearrange the raw data without outliers, result in sorted_data
	//sort_raw_data_without_outliers();
...
//sort m_data without outliers after bkm tree is built.
template<typename T>
void Convex_Tree<T>::sort_raw_data_without_outliers(){
	register int i, j, nodes_num = this->nodes.size();
	int m_sorted_data_len = m_data.nrows() - all_exc_data.nrows();
	this->m_sorted_data.resize(m_sorted_data_len, m_data.ncols());
	this->m_sorted_data_ori_indexes.resize(m_sorted_data_len);
	int cur_offset = 0;
	int aligned_data_len = 0;
	//int cur_leave_indx  =0;
	//std::set<unsigned int> indexes();
...
void Convex_Tree<T>::align_sorted_raw_data(int aligned_len){
    ...
    //this->m_aligned_sorted_data.print();
    //this->m_aligned_sorted_data_ori_indexes.print("  ");
...
void  Convex_Tree<T>::build_simplified_tree(){
	...
	this->nodes_centers.resize(this->nodes_num, DIM);
    ...
    for (int i=0; i<this->nodes_num;i++){
		...
		//the root node has no initialized center
		if (i > 0){
			//this->nodes[i]->center.print("center:");
			this->nodes_centers.setRow(i, this->nodes[i]->center);
			//this->nodes_centers.extractRow(i).print("the first row is:");
		}
    }
}
...
//Update leaf nodes by FastDPeak 
template <typename T>
void Convex_Tree<T>::Update_leaf_nodes(){
	int Exc_data_index[10000], exc_n, all_exc_n = 0, c = 0, data_n = 0;
	int max_indexes = this->m_data.nrows();
	stack<int> Exc_index;
	for (int i = 0; i < nodes_num; i++){
		if (this->nodes[i]->isLeaf){
			int dim = this->m_data.ncols();
			int data_size = this->nodes[i]->data.nrows();
			if (data_size < 50)   continue;
			T* data_with_index = (T*)malloc((dim + 1)*data_size*sizeof(T));
			for (int j = 0; j < data_size; j++){
				for (int k = 0; k < dim; k++){
					data_with_index[j*(dim+1) + k] = this->nodes[i]->data.get_v_i_j(j, k);
				}
				data_with_index[(j+1)*(dim+1)-1] = j;
			}
			//Finding outliers from leaf data by FastDPeak
			exc_n = 0;
			int FastDPeak_K = 30,L_P_T = floor(data_size * 0.05);
			float *dis_matrix = (float*)malloc(3 * data_size * FastDPeak_K * sizeof(float));
			node_p *node_p_ptr = new node_p[data_size];
			Fast_Density_Peak(FastDPeak_K, data_with_index, data_size, data_size, dim, 
				              Exc_data_index, &exc_n, L_P_T, dis_matrix, node_p_ptr);
			free(data_with_index); free(dis_matrix);
			//std::cout << "the node " << i << " info:" << endl;
			//std::cout << "the number of raw data: " << this->nodes[i]->data.nrows() << endl;
			//std::cout << "the number of outliers: " << exc_n << endl;

			//Updating the information of leaf point:
			//  1. Matrix<T> data: data saves the points within it
			//  2. int saved_data_offset: the off_set of the data saved in sorted_data in bkm tree
			//  3. int pts_number: the number of points
			//  4. Vector<T> pts_indexes: the index of points
			//  5. Vector<FLOAT_TYPE> BETA: the BETA of the leaf node
			
			int raw_pts_number = this->nodes[i]->pts_number;
			int pts_number = raw_pts_number - exc_n;
			//std::cout << "exc_n:" << exc_n << endl;
			//std::cout << "pts_number:" << pts_number << endl;
			this->nodes[i]->pts_number = pts_number;

			int* new_data_indexes = (int*)malloc(pts_number*sizeof(int));
			int p_index = 0, exc_index = 0;
			for (int j = 0; j < pts_number || exc_index < exc_n;){
				if (exc_index < exc_n){
					if (p_index == Exc_data_index[exc_index]){
						Exc_index.push(this->nodes[i]->pts_indexes.extract(p_index));
						p_index++; exc_index++; all_exc_n++;  continue;
					}
				}
				new_data_indexes[j] = this->nodes[i]->pts_indexes.extract(p_index);
				j++; p_index++;
			}
			Vector<int> pts_indexes(new_data_indexes, pts_number);
			this->nodes[i]->pts_indexes = pts_indexes;
			/*
			if (c >= 0){
				this->nodes[i]->pts_indexes.print("\nthe index of data without outliers:");
			}*/
			//std::cout << "the number of data without outliers: " << this->nodes[i]->pts_indexes.size() << endl;
			//std::cout << "the size of all outliers: " << Exc_index.size() << endl;
			free(new_data_indexes);
			this->nodes[i]->data = this->m_data.extractRows(pts_indexes);
			pts_indexes.destroy();

			//updating beta of leaf node
			int alpha_num = this->nodes[i]->ALPHA.nrows();
			for (int j = 0; j < alpha_num; j++){
				Vector<T> alpha_vec(this->nodes[i]->ALPHA[j],dim);
				T new_beta = this->getBeta_1(alpha_vec, nodes[i]->BETA[j], nodes[i]->pts_indexes);
				this->nodes[i]->BETA[j] = new_beta;
			}
			
			//std::cout << "finish update the information of leaf " << endl;
			c++;
		}
	}
	/*for (int i = 0; i < nodes_num; i++){
		if (this->nodes[i]->isLeaf){
			data_n += this->nodes[i]->pts_indexes.size();
		}
	}
	std::cout << "the nuber of raw data without outliers: " << data_n << endl;
	*/
	int* temp = (int*)malloc(all_exc_n*sizeof(int));
	int count = 0;
	while (!Exc_index.empty()){
		temp[count++] = Exc_index.top();
		Exc_index.pop();
	}
	sort(temp, temp + all_exc_n);
	/*
	std::cout << "the index of outliers: ";
	for (int i = 0; i < all_exc_n; i++){
		std::cout << temp[i] << ", ";
	}
	std::cout << endl;
	*/
	Vector<int> all_exc_index(temp, all_exc_n);
	this->all_exc_indexes = all_exc_index;
	this->all_exc_data = this->m_data.extractRows(all_exc_index);
	std::cout << "the number of outlier:" << all_exc_n << endl;
	//all_exc_index.print("the index of all outliers");
}
...
//virtual procedure
template <typename T>
void Convex_Tree<T>::save_KNN_running_time_info(FILE* log_file, int query_num)
{
};

//virtual procedure
template <typename T>
void Convex_Tree<T>::new_save_KNN_running_time_info(FILE* log_file, int query_num)
{
};
