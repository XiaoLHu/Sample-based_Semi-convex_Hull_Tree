    ...
    class CPU_Convex_Tree :public Convex_Tree<T>
{
    public:
        ...
        virtual ~CPU_Convex_Tree(){
            //free  cur_k_NNResult
			int len=this->cur_k_NNResult.size();
			
			//free the whole vector
			NNResult<T>::free_an_NNResult_vec(this->cur_k_NNResult);
			/*
			//the following code will failed in VC
			for (int i = 0; i<len; i++){
                //this->cur_k_NNResult[i]->free_stru();
				NNResult<T>* tmp = (NNResult<T>*)this->cur_k_NNResult[i];
				tmp->free_stru();
            }
			*/
    ...
    protected:
        ...
        */
		void virtual init_kNN_result(Matrix<T>& query_points_mat);
    ...
    private:
		...
		void kNN_cpu_by_leaf(Matrix<T>& query_points_mat);
		void kNN_cpu_recurisive(Matrix<T>& query_points_mat);
		...
		void do_kNN_recurisive_by_loop(Vector<T>&q, int query_point_index);
...
//override
template <typename T>
void CPU_Convex_Tree<T>::init_kNN_result(Matrix<T>& query_points_mat){
	...
	this->appr_leaf_node_indexes.resize(query_points_mat.nrows());
};
...
void CPU_Convex_Tree<T>::do_kNN(Matrix<T> &query_points_mat){
    ...
    if (this->ALG_TYPE==USE_CPU_LEAF_APPRXIMATE_QP){
		kNN_cpu_by_leaf(query_points_mat);
    }

    if (this->ALG_TYPE==USE_CPU_RECURSIVE_APPRXIMATE_QP){
		kNN_cpu_recurisive(query_points_mat);
    }
}
...
template <typename T>
void CPU_Convex_Tree<T>::kNN_cpu_by_leaf(Matrix<T>& query_points_mat){
	for (int i = 0; i<query_points_mat.nrows(); i++){
...
template <typename T>
void CPU_Convex_Tree<T>::kNN_cpu_recurisive(Matrix<T>& query_points_mat){
	for (int i = 0; i<query_points_mat.nrows(); i++){
		Vector<T>& q = query_points_mat.extractRow(i);
        this->approximate_kNN(q,i);
        //do_kNN_recurisive(0, i);
		do_kNN_recurisive_by_loop(q, i);
    }
...
template <typename T>
void CPU_Convex_Tree<T>::do_kNN_recurisive_by_loop(Vector<T>&q, int query_point_index){

    //int visiting_times_recorder[this->nodes_num];
	int visiting_times_recorder[1000000];
