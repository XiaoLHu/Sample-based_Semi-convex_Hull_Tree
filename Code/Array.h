...
template <typename T> class v_array;
...
public:
	...
	inline T extract(const unsigned int index) const;
...
template <typename T>
inline T Vector<T>::extract(const unsigned int index) const
{
	return v[index];
}
...
template <typename T>
inline T vec_max(const Vector<T>& v)
{
	T value = v[0];
...
template <typename T>
inline T vec_min(const Vector<T>& v)
{
	T value = v[0];
	for (unsigned int i = 1; i < v.size(); i++){
		value = ((v[i] < value) ? v[i] : value);
	}
...
public:
	...
	static Matrix<T> new_rand_matrix(const unsigned int n, const unsigned int m, const T* min, const T* max);
	...
	inline T get_v_i_j(unsigned int i, unsigned int j) const;
...
template <typename T>
inline T Matrix<T>::get_v_i_j(unsigned int i, unsigned int j) const{
	return this->v[i][j];
}
...
template <typename T>
Matrix<T> Matrix<T>::new_rand_matrix(const unsigned int n, const unsigned int m, const T* min, const T* max){
	Matrix<T> result(n, m);
	int min_tmp, max_tmp;
	for (int i = 0; i<n; i++){
		for (int j = 0; j<m; j++){
			min_tmp = (int)min[j];
			max_tmp = (int)max[j];
			result[i][j] = rand()%(max_tmp-min_tmp+1) + min_tmp;
		}
	}
	return result;
}
...
template <typename T>
Vector<T> mat_max(const Matrix<T>& m)
{
...
template <typename T>
Vector<T> mat_min(const Matrix<T>& m)
{
	Vector<T> res(m.ncols());
	double value;
	for (unsigned int j = 0; j < m.ncols(); j++)
