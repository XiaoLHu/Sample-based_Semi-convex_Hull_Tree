	...
	v_array<v_array<ds_node> >& stack)
{
	if (point_set.index == 0)
		...
		{
			v_array<ds_node> far_ = pop(stack);
			split(point_set, far_, max_scale); //O(|point_set|)
			...
			if (point_set.index == 0)
			{
				...
				while (point_set.index != 0) { //O(|point_set| * num_children)
					...
					dist_split(far_, new_point_set, new_point, max_scale); //O(|far|)
